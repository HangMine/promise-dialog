...
import { ComponentProps, CreateParams, DialogFC, ModalProps, UnkownFC } from './types';
import {
  DialogAction,
  dialogActions,
  DialogStore,
...
export class Dialog {
  ...
  static open<
    R,
    T extends DialogFC<P> | React.ReactNode = any,
    P extends ComponentProps = any,
  >(
  ...
  create<T extends DialogFC<P> | React.ReactNode, P extends ComponentProps>(
    ...
    if (Dialog.isReactNode(ReactNodeOrComponent)) {
      this.createByReactNode(
        ...
        componentPropsOrModalProps as ModalProps,
      );
    } else {
      this.createByComponent(
        ReactNodeOrComponent,
        componentPropsOrModalProps as P,
        modalProps,
      );
    }
    // 创建的时候才保存
